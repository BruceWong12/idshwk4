...
{
    # Make observations on the following types of responses
    # 1. all response
    SumStats::observe("http_all_resp", 
    ...
    if (code == 404){
        # 2. 404 response
        SumStats::observe("http_404_resp", 
                        SumStats::Key($host = c$id$orig_h), 
                        SumStats::Observation($num = 1)
                        );
        # 3. 404 url response
        SumStats::observe("http_url_404_resp", 
                        SumStats::Key($host = c$id$orig_h), 
    ...
    SumStats::create([$name = "scanner",
                    ...
                    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) =
                    {
                        if("http_all_resp" in result && "http_404_resp" in result && "http_url_404_resp" in result)
                        {
                            ...
                            if(cnt_http_all_resp > 2 # rule 1
                            && cnt_http_404_resp > 0.2 * cnt_http_all_resp #rule 2
                            && cnt_http_url_404_resp > 0.5 * cnt_http_404_resp #rule 3
                            )
                        	{
